...
public class MainActivity extends Activity {
    ...
    private static final String PREFS_NAME = "salud_urs_blut_storage_v1_neu";
    ...
    private void buildUi() {
        ...
        subtitle.setText("Blutdruck-App · Version 1.0 Neuaufbau · ohne OCR");
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (currentPhotoFile == null || photoUri == null) {
            statusText.setText("Foto konnte nicht gespeichert werden: Datei fehlt.");
            return;
        }

        if (!currentPhotoFile.exists() || currentPhotoFile.length() == 0) {
            statusText.setText("Foto wurde nicht korrekt gespeichert. Bitte nochmals versuchen.");
            return;
        }

        try {
            Bitmap bitmap = decodeSampledBitmap(currentPhotoFile.getAbsolutePath(), 1200, 1200);
            if (bitmap != null) {
            ...
            statusText.setText("Foto gespeichert, aber Vorschau zu groß. Werte können trotzdem gespeichert werden.");
        } catch (Exception e) {
            statusText.setText("Foto gespeichert, Vorschau konnte aber nicht geladen werden: " + safeError(e));
        }
    }
    ...
    private Bitmap decodeSampledBitmap(String path, int reqWidth, int reqHeight) {
        try {
            ...
            int sampleSize = 1;
            while ((bounds.outHeight / sampleSize) > reqHeight || (bounds.outWidth / sampleSize) > reqWidth) {
                sampleSize *= 2;
            }

...
const PageHeaderContent = () => {
  ...
  const title = headerNavigationList.find((item) => pathName.includes(item.link))?.label;
  return (
    <div className={styles.wrapper}>
      <Container className="flex flex-col tablet:flex-row items-center justify-between gap-4 tablet:gap-0">
        <h1>{dictionary[title ?? "constructionDescription"]}</h1>
        <div className="flex items-center">
          <a href="/" className="text-green">
            {dictionary["home"]}
          </a>
          <Icon name="ArrowRight" className="text-secondaryGray w-4 h-4" />
          <p className="text-secondaryGray">{dictionary[title ?? "constructionDescription"]}</p>
        </div>
      </Container>
    </div>
  );
